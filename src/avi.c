/*
**  This code based on Oricutron, ORIC Emulator.
...
static Uint32 get_time_milliseconds(void)
{
	struct timeval te; 
	Uint32 milliseconds;
...
static int write32l( int stillok, struct avi_handle *ah, Uint32 val, Uint32 *rem )
{
  if( !stillok ) return 0; // Something failed earlier, so we're aborting
  if( rem ) (*rem) = ah->csize;
  ah->csize += 4;
  _MAKE_LE32(val);
...
static int write16l( int stillok, struct avi_handle *ah, Uint16 val, Uint32 *rem )
{
  if( !stillok ) return 0; // Something failed earlier, so we're aborting
  if( rem ) (*rem) = ah->csize;
  ah->csize += 2;
...
static int writestr( int stillok, struct avi_handle *ah, char *str, Uint32 *rem )
{
...
static int seek_write32l( int stillok, struct avi_handle *ah, Uint32 offs, Uint32 val )
{
  if( !stillok ) return 0; // Something failed earlier, so we're aborting
  if( offs == 0 ) return 1; // Don't overwrite RIFF!
...
static int writeblk( int stillok, struct avi_handle *ah, Uint8 *blk, Uint32 blklen )
{
  if( !stillok ) return 0;
  ah->csize += blklen;
...
static int writebyt( int stillok, struct avi_handle *ah, Uint8 val )
{
  if( !stillok ) return 0; // Something failed earlier, so we're aborting
  ah->csize += 1;
...
struct avi_handle *avi_open( char *filename, Uint8 *palette, int dosound, int is50hz )
{
  ...
  ok &= write32l( ok, ah,            320, NULL               );   // Width
  ok &= write32l( ok, ah,            200, NULL               );   // Height
  ...
  ok &= write32l( ok, ah,        320*200, NULL               );   // Image size
  ...
  for( i=0; i<8; i++ )
  {
    ok &= writebyt( ok, ah, palette[i*3+2] );
    ok &= writebyt( ok, ah, palette[i*3+1] );
    ok &= writebyt( ok, ah, palette[i*3+0] );
    ok &= writebyt( ok, ah, 0 );
  }
...
int avi_addframe( struct avi_handle **ah, Uint8 *srcdata )
{
  ...
  if( (*ah)->lastframevalid )
  {
    ...
    for( i=0; i<200; i++ )
    {
      for( x=0; x<320; x++ )
      {
        if( (*ah)->lastframe[i*320+x] != srcdata[i*320+x] )
          break;
      }
      if( x<320 ) break;
    }
  ...
  if( lastline == 200 )
  {
    ...
    for( i=199,size=0; i>=lastline; i-- )
    {
      ...
      size = rle_encode( *ah, &srcdata[i*320], size, lastx );
    }
    (*ah)->rledata[size-1] = 0x01;
  }

  memcpy( (*ah)->lastframe, srcdata, 320*200 );
  (*ah)->lastframevalid = 1;
  ...
  if( !(*ah)->time_start )
  {
    (*ah)->time_start = get_time_milliseconds();
  }
...
void avi_close( struct avi_handle **ah )
{
  ...
  time_end = get_time_milliseconds();
...
{
	vidcap = avi_open("/tmp/test.avi", oricpalette, 0, 1);
	do_video_capture = 1;
}
